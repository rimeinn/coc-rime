...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  rime = new Rime(new Session(await userConfig.traits), userConfig.ui);
  isEnabled = userConfig.enabled;
  const statusBarItem = window.createStatusBarItem(0, {progress: false});
  let schemaId = rime.session.get_current_schema();
  if (schemaId !== userConfig.schemaId && userConfig.schemaId !== '') {
    rime.session.select_schema(userConfig.schemaId);
    schemaId = userConfig.schemaId;
  }
  statusBarItem.text = userConfig.shortcut + ' ' + rime.session.get_schema_list().filter((schema) => {
        ...
        provideCompletionItems(
          ...
          if (offset !== 0 && isEnabled) {
            ...
            if (punctMap.has(singleChar)) {
              ...
              const res = rime.session.getFullContext(inputString);
              if (
                res !== null &&
                'composition' in res &&
